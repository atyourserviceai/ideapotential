...
/**
 * Get the complete details of the current idea including all assessment data
 */
export const getCurrentIdeaDetails = tool({
  description: "Get the complete details of the currently selected idea including all factor scores, evidence, and assessment data",
  parameters: z.object({}),
  execute: async () => {
    const { agent } = getCurrentAgent<AppAgent>();
    if (!agent) {
    ...
    try {
      ...
      if (!currentIdea) {
        return "No idea is currently selected. Please select an idea or start a new assessment.";
      }
      
      // Calculate progress summary
      const checklist = currentIdea.checklist || {};
      const factors = Object.entries(checklist);
      const scoredFactors = factors.filter(([_, item]) => item.score !== null && item.score !== undefined);
      const potentialFactors = ['problem_clarity', 'market_pain_mentions', 'outcome_gap', 'competitive_moat', 'team_solution_fit', 'solution_evidence', 'team_market_fit'];
      const actualizationFactors = ['early_demand', 'traffic_authority', 'marketing_product_fit'];
      
      const scoredPotential = scoredFactors.filter(([key, _]) => potentialFactors.includes(key));
      const scoredActualization = scoredFactors.filter(([key, _]) => actualizationFactors.includes(key));
      
      // Calculate derived scores
      const potentialScore = scoredPotential.length > 0 ? 
        Math.round((scoredPotential.reduce((sum, [_, item]) => sum + (item.score || 0), 0) / (potentialFactors.length * 5)) * 100) : 0;
      const actualizationScore = scoredActualization.length > 0 ? 
        Math.round((scoredActualization.reduce((sum, [_, item]) => sum + (item.score || 0), 0) / (actualizationFactors.length * 5)) * 100) : 0;
      
      const unscored = factors.filter(([_, item]) => item.score === null || item.score === undefined).map(([key, _]) => key);
      const lowestScoring = scoredFactors.length > 0 ? 
        scoredFactors.sort(([_, a], [__, b]) => (a.score || 0) - (b.score || 0)).slice(0, 3).map(([key, _]) => key) : [];
      
      return {
        idea: {
          id: currentIdea.idea_id,
          title: currentIdea.title,
          oneLiner: currentIdea.one_liner,
          description: currentIdea.description,
          stage: currentIdea.stage,
          founderBackground: currentIdea.founder_background,
        ...
        assessment: {
          ...
          scoredFactors: scoredFactors.length,
          progressPercentage: Math.round((scoredFactors.length / factors.length) * 100),
          unscoredFactors: unscored,
          lowestScoringFactors: lowestScoring
        },
        checklist: currentIdea.checklist,
        conversationInsights: currentIdea.conversation_insights || [],
        nextSteps: unscored.length > 0 ? 
          `Focus on scoring unscored factors: ${unscored.slice(0, 3).join(', ')}` :
          lowestScoring.length > 0 ? 
          `Improve lowest-scoring factors: ${lowestScoring.join(', ')}` :
          'Assessment appears complete - review and provide final recommendations'
      };
    } catch (error) {
      console.error("Error in getCurrentIdeaDetails:", error);
      return `Error getting current idea details: ${error instanceof Error ? error.message : "Unknown error"}`;
    }
  },
});
